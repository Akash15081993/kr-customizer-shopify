...
import { searchProductsById } from "@/lib/product/searchProductsById";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!shop || !rowId || !productId || !metafield) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: shop, accessToken, rowId, productId, and metafield are required",
        },
        { status: 400 }
      );
    }
    ...
    //Fetch product by id
    const product = await searchProductsById(shop, accessToken, productId)
    
    //Add recode in main table
    const payload = {
      "storeHash" : (shopInfo as any)?.id?.toString(),
      "id":rowId, //Database ID
      "productSku" : "-",
      "productName" : product?.title,
      "productImage" : product?.featuredImage ? product?.featuredImage?.url : null,
      apiToken: langEng?.storeApi?.token + "-product-update",
    }

    await fetch(`${langEng?.storeApi?.endPoint}product/update`, {
      method: "PUT",
