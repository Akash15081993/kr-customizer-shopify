...
// Add this new function for GDPR mandatory webhooks
export async function RegisterGDPRWebhooks(shop: string, accessToken: string) {
  const apiVersion = getCurrentShopifyVersion();
  ...
  try {
    // Get all existing webhooks first
    const getResponse = await fetch(`https://${shop}/admin/api/${apiVersion}/webhooks.json`, {
      headers: {
        "X-Shopify-Access-Token": accessToken,
        "Content-Type": "application/json",
    ...
    for (const webhook of gdprWebhooks) {
      // Check if this GDPR webhook already exists
      const existingWebhook = existingWebhooks.find(
        (wh: any) => wh.topic === webhook.topic && wh.address === webhook.address
      );

      if (existingWebhook) {
        console.log(`GDPR webhook ${webhook.topic} already exists`);
        continue;
      }

      // Check if webhook exists with same topic but different URL
      const topicWebhook = existingWebhooks.find((wh: any) => wh.topic === webhook.topic);
      ...
      if (topicWebhook) {
        ...
        console.log(`Updated GDPR webhook: ${webhook.topic}`);
      } else {
        // Create new webhook
        ...
        console.log(`Created GDPR webhook: ${webhook.topic}`);
      }
    }
  } catch (error) {
    console.error("Error registering GDPR webhooks:", error);
    throw error;
  }
}